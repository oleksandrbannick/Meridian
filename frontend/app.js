...
function _renderLadderArbCard(bot, botId, container, gameScores, gameKey) {
    ...
    if (hasInv) {
        ...
        if (exitPrice > 0) {
            ...
            exitLineHtml = `<div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px;padding-top:6px;border-top:1px solid #1a2540;">
                <div style="display:flex;align-items:center;gap:6px;">
                    <span style="color:#3a8a9a;font-size:10px;font-weight:700;">EXIT ${exitSide} @${exitPrice}c</span>
                    ${walkLabel}
                    <span style="color:#445;font-size:9px;">${bidDist}</span>
                </div>
                <span style="color:#3a8a9a;font-size:10px;font-weight:700;">${exitFillQty}/${exitTotalQty}</span>
            </div>
            <div style="height:3px;background:#0a1018;border-radius:2px;overflow:hidden;margin-top:3px;">
                <div style="width:${fp}%;height:100%;background:#3a8a9a;border-radius:2px;transition:width .3s;"></div>
            </div>`;
        }
    ...
    const _rungHtml = (price, level, sideCol) => {
        ...
        const barCol = isFull ? sideCol : '#3a8a9a';
        const textCol = isFull ? sideCol : active ? '#3a8a9a' : '#334';
        const statusTag = isFull ? `<span style="color:${sideCol};font-size:7px;font-weight:800;letter-spacing:.04em;">FILLED</span>`
            : active ? `<span style="color:#3a8a9a;font-size:7px;font-weight:700;">LIVE</span>`
        ...
        historyHtml = `<div style="border-top:1px solid #1a2540;margin-top:4px;padding-top:4px;">
            <div onclick="const el=document.getElementById('${historyId}');el.style.display=el.style.display==='none'?'block':'none';this.querySelector('.arr').textContent=el.style.display==='none'?'\\u25B6':'\\u25BC';" style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;padding:2px 0;">
                <div style="display:flex;gap:10px;font-size:10px;">
                    ...
                    <span style="color:#3a8a9a;">${roundTrips} RTs</span>
                </div>
            ...
            <div id="${historyId}" style="display:none;margin-top:4px;">
                ${rtLog.length > 0 ? rtLog.map((rt, i) => {
                    ...
                    return `<div style="display:flex;justify-content:space-between;padding:2px 0;font-size:10px;${i > 0 ? 'border-top:1px solid #0f1520;' : ''}">
                        ...
                        <span><span style="color:#00d4ff;">${rt.entry_price}</span><span style="color:#334;">+</span><span style="color:#3a8a9a;">${rt.exit_price}</span><span style="color:#334;">=</span><span style="color:${combCol};font-weight:700;">${rt.combined}c</span></span>
                        <span style="color:#3a8a9a;">x${rt.qty}</span>
    ...
    item.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
            ...
            <div style="display:flex;align-items:center;gap:5px;flex-shrink:0;">
                ...
                ${!isCompleted ? `<button onclick="apexMmModify('${botId}')" style="background:#3a8a9a15;color:#3a8a9a;border:1px solid #3a8a9a30;border-radius:5px;padding:3px 7px;font-size:9px;cursor:pointer;font-weight:700;">Edit</button>` : ''}
        ...
        ${!isCompleted ? `<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:4px;">
            <div style="background:#060a12;border:1px solid ${yesIsExit ? '#00ff8820' : '#3a8a9a15'};border-radius:6px;padding:6px 8px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                    <span style="color:${yesIsExit ? '#00ff88' : '#3a8a9a'};font-size:8px;font-weight:800;letter-spacing:.06em;">${yesLabel}${yesPaused ? ' <span style="color:#ffaa00;">PAUSED</span>' : ''}</span>
            ...
            <div style="background:#060a12;border:1px solid ${noIsExit ? '#ff444420' : '#3a8a9a15'};border-radius:6px;padding:6px 8px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                    <span style="color:${noIsExit ? '#ff4444' : '#3a8a9a'};font-size:8px;font-weight:800;letter-spacing:.06em;">${noLabel}${noPaused ? ' <span style="color:#ffaa00;">PAUSED</span>' : ''}</span>
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;padding-top:4px;font-size:9px;color:#334;">
            <span style="color:#3a8a9a;">W:${width}</span>
            <span>Mid:${midpoint}</span>
            <span style="color:#3a8a9a;">${bot.base_qty || bot.qty_per_level || '?'}x${bot.levels || '?'}${bot.auto_scale !== false ? ' scaled' : ''}</span>
...
async function showBotDetail(botId) {
    ...
    try {
        ...
        if (isPhantom) {
            ...
            html += `<div>Combined: <strong>${combined}¢</strong> <span style="color:#555;">(${100-combined}¢ profit)</span></div>`;
...
async function apexMmModify(botId) {
    ...
    const html = `
        <div style="background:#0f1419;border:1px solid #00d4ff44;border-radius:12px;padding:20px;max-width:320px;">
            ...
            <div style="display:flex;gap:8px;">
                <button onclick="apexMmModifySave('${botId}')" style="flex:1;background:linear-gradient(135deg,#00d4ff,#3a8a9a);color:#000;border:none;border-radius:6px;padding:8px;font-size:12px;font-weight:700;cursor:pointer;">Save</button>
