...
if [ "$LOCAL" != "$REMOTE" ]; then
    ...
    git checkout origin/meridian -- backend/app.py frontend/ backend/requirements.txt backend/restart.sh auto-deploy.sh 2>/dev/null
    # Update HEAD to match remote so we don't re-trigger
    git reset --soft origin/meridian 2>/dev/null
    # Restart via systemd (clean, no port conflicts)
