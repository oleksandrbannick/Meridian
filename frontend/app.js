...
function displayOrderbookLadder(orderbook) {
    ...
    const catchLabel = catchScore >= 90 ? 'WALL' : catchScore >= 75 ? 'PRIME' : catchScore >= 55 ? 'SNIPER' : catchScore >= 45 ? 'TRAP' : catchScore >= 40 ? 'DEEP' : catchScore >= 35 ? 'FLOOR' : 'KILL';
    const catchCol = catchScore >= 90 ? '#00ff88' : catchScore >= 75 ? '#00ccff' : catchScore >= 55 ? '#ffaa00' : catchScore >= 45 ? '#ff8800' : catchScore >= 40 ? '#ff6600' : catchScore >= 35 ? '#ff4400' : '#ff4444';
    ...
    const depthHtml = `<div style="background:#0f1419;border:1px solid #1e2740;border-radius:8px;padding:10px;margin-bottom:12px;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:4px 5px;margin-bottom:6px;font-size:9px;font-weight:700;justify-content:center;">
            <span style="color:#00ff88;background:#00ff8815;padding:2px 6px;border-radius:4px;">90+ WALL 3¢</span>
            <span style="color:#00ccff;background:#00ccff15;padding:2px 6px;border-radius:4px;">75+ PRIME 4¢</span>
            <span style="color:#ffaa00;background:#ffaa0015;padding:2px 6px;border-radius:4px;">55+ SNIPER 5¢</span>
...
function updateAnchorPreview() {
    ...
    if (shaveInfo) {
        ...
        if (_obCache && (_obCache._backendRec || _obCache.dogDepth > 0)) {
            ...
            const _ppiTier = _obCache.ppiTier || (_ppiScore >= 90 ? 'WALL' : _ppiScore >= 75 ? 'PRIME' : _ppiScore >= 55 ? 'SNIPER' : _ppiScore >= 45 ? 'TRAP' : _ppiScore >= 40 ? 'DEEP' : _ppiScore >= 35 ? 'FLOOR' : 'KILL');
            const _ppiTierCol = _ppiScore >= 90 ? '#00ff88' : _ppiScore >= 75 ? '#00ccff' : _ppiScore >= 55 ? '#ffaa00' : _ppiScore >= 35 ? '#ff8800' : '#ff4444';
            const _creBaseD = _ppiScore >= 90 ? 3 : _ppiScore >= 75 ? 4 : _ppiScore >= 55 ? 5 : _ppiScore >= 45 ? 6 : _ppiScore >= 40 ? 7 : _ppiScore >= 35 ? 8 : 0;
    ...
    item.innerHTML = `
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:8px;padding-top:8px;border-top:1px solid #1e2740;font-size:10px;">
            <span style="color:#ff66aa;font-weight:600;">Depth: ${bot.anchor_depth || targetWidth}¢${bot.auto_depth ? ' <span style="color:#64ffda;font-size:8px;">AUTO</span>' : ''}</span>${bot._rec_qty != null ? (() => { const _rq = bot._rec_qty, _mq = bot._max_qty || _rq; const _ok = qty <= _rq; const _over = qty > _mq; const _border = _ok ? '#00ff8833' : _over ? '#ff444433' : '#ffaa0033'; const _recCol = qty <= _rq ? '#00ff88' : '#ff4444'; return `<span style="display:inline-flex;align-items:center;border:1px solid ${_border};border-radius:4px;overflow:hidden;font-size:9px;font-weight:700;" title="Fav bid L1: ${bot._fav_bid_l1 || '?'} · Fav ask L1: ${bot._fav_ask_l1 || '?'}"><span style="padding:1px 5px;color:#b2ff59;background:#b2ff5910;">×${qty}</span><span style="padding:1px 5px;color:${_recCol};background:${_recCol}10;border-left:1px solid #1e2740;">rec ${_rq}</span><span style="padding:1px 5px;color:#ff4444;background:#ff444410;border-left:1px solid #1e2740;">max ${_mq}</span></span>`; })() : `<span style="color:#b2ff59;">×${qty}</span>`}${(() => {
                ...
                if (_ppi != null) {
                    const _ppiCol = _ppi >= 90 ? '#00ff88' : _ppi >= 75 ? '#00ccff' : _ppi >= 55 ? '#ffaa00' : _ppi >= 45 ? '#ff8800' : _ppi >= 40 ? '#ff6600' : _ppi >= 35 ? '#ff4400' : '#ff4444';
                    const _ppiLabel = _ppi >= 90 ? 'WALL' : _ppi >= 75 ? 'PRIME' : _ppi >= 55 ? 'SNIPER' : _ppi >= 45 ? 'TRAP' : _ppi >= 40 ? 'DEEP' : _ppi >= 35 ? 'FLOOR' : 'KILL';
                    const _baseD = _ppi >= 90 ? 3 : _ppi >= 75 ? 4 : _ppi >= 55 ? 5 : _ppi >= 45 ? 6 : _ppi >= 40 ? 7 : _ppi >= 35 ? 8 : 0;
        ...
        depthPanel.innerHTML = depths.length === 0 ? '' : `
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:8px;">
                ${depths.map(d => {
                    ...
                    const _tierLabel = d.depth <= 3 ? 'WALL' : d.depth <= 4 ? 'PRIME' : d.depth <= 5 ? 'SNIPER' : d.depth <= 6 ? 'TRAP' : d.depth <= 7 ? 'DEEP' : d.depth <= 8 ? 'FLOOR' : '';
                    const _tierCol = d.depth <= 3 ? '#00ff88' : d.depth <= 4 ? '#00ccff' : d.depth <= 5 ? '#ffaa00' : d.depth <= 6 ? '#ff8800' : d.depth <= 7 ? '#ff6600' : d.depth <= 8 ? '#ff4400' : '#555';
