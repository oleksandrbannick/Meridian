...
function getTeamLogoHtml(code, size = 20, sportHint = '') {
    ...
    // When a sportHint is given, NEVER cross-sport guess.
    // DAL exists in NBA/NFL/NHL/MLB sets — without this guard, an MLS Dallas card
    // would render the NBA Mavericks logo. If the hint's sport doesn't have the
    // code, use the letter badge instead.
    let sport = '';
    if (!isCollege) {
        if (hint) {
            if (_ESPN_LOGO_SPORTS[hint] && _ESPN_LOGO_SPORTS[hint].has(upper)) {
                sport = hint;
            }
        } else {
...
function applyFilters() {
    // Refresh Kalshi↔ESPN soccer aliases before render — defensive against
    // pre→in transitions that happen between loadLiveScores ticks (game went
    // live AFTER the last alias pass, so liveGames lookups would still miss)
    injectKalshiSoccerAliases();
...
function displayEventRow(eventData, container) {
    ...
    if (gameScore && !(gameScore.state === 'pre' && kalshiLive)) {
        ...
        // — the lookup will always miss for these, so flag them explicitly.
...
function displayOrderbookLadder(orderbook) {
    ...
    // ── PHANTOM PRECISION INDEX (v6b restored 2026-04-26) ──
    // Reverted from v10 — Apr 16-19 v6 era was peak performance. Back to D+G+S+T weighting.
    const _favPL = favAnalysis.perLevel;
    const _obTk = ob._ticker || orderbook?.ticker || '';
    const _scoreSport = detectSport(_obTk);

    // 1. DENSITY (40pts) — original v6 curve, max at fpl ≥100k
    ...
    // 2. GAP COUNT (25pts) — v6 original linear monotonic reward
    const _favGapCount = favAnalysis.gaps;
    const gapPts = Math.min(25, _favGapCount * 5);
    const gapPenalty = gapPts;

    // 3. SPREAD (20pts) — v6 original: tight spread = max
    ...
    if (_gStatus === 'in') {
        ...
        else if (_gPeriod >= _maxPeriod - 1) timePts = 5;
        else if (_gPeriod >= Math.ceil(_maxPeriod / 2)) timePts = 10;
    ...
    if (catchScore >= 90) {
        ...
        if (_favIsThin) {
            ...
            verdict = `${_tierName} — ${favAnalysis.gaps} fav gaps, unreliable hedge. ${_recDepth}¢+ depth`;
        } else {
            verdict = `${_tierName} — low liquidity or late game. ${_recDepth}¢+ depth if trading`;
        }
        verdictCol = catchScore >= 40 ? '#ff8800' : '#ff4400';
    } else {
        if (_favIsThin) {
...
function setTradeMode(mode) {
    ...
    if (mode === 'straight') {
        ...
        // ALWAYS default to AUTO on modal open. localStorage no longer persists
        // autoDepth=false — the prior behavior caused users to launch bots with
        // auto off when they thought it was on (button visual state lagged after
...
function initAnchorDogPrices() {
    ...
    // Phantom ALWAYS posts from bid - depth, even in broken/wide markets.
    // Old broken-spread fallback (post at ask − depth) was a legacy concept that
    // produced 85¢ posts on heavy-fav markets — wrong, phantom dog is bid-side only.
    _anchorIsBrokenSpread = false;
    const anchorBase = _anchorDogBid;
                ...
                .then(data => {
                    if (data && data.rec_depth) {
                        ...
                        window._obDepthCache[_obTicker] = {
                            ...
                            favConc: data.fav_concentration, maxSafeQty: data.max_safe_qty, suggestedQty: data.suggested_qty,
                            favTop3: 0, dogWall: {},
...
function _renderDogBotCard(bot, botId, container, gameScores) {
    ...
    // PARKED only when dog is actually near the 40c ceiling — _parked_at_ceiling
    // can stick stale across cycles when bot gets pulled before the clear path runs.
    const _isParked = status === 'dog_anchor_posted' && bot._parked_at_ceiling && (bot.dog_price || 0) >= 38;
    ...
    const statusMap = {
        ...
        'fav_hedge_posted': '⭐ HEDGE POSTED', 'waiting_repeat': _isAnyPulled ? _pullLabel : bot._just_completed ? '✅ COMPLETED' : bot._flip_pending ? '⚡ FLIPPING' : '🔄 REPEATING',
    ...
    const borderMap = {
        ...
        'fav_hedge_posted': _phPink, 'waiting_repeat': _isAnyPulled ? _phPurple : bot._just_completed ? _phGreen : _phAmber,
    ...
    item.innerHTML = `
        ...
        ${bot._stop_pending && !_isCompletedSummary ? `<div style="background:#ff880012;border:1px solid #ff880033;border-radius:6px;padding:4px 10px;margin-bottom:6px;">
...
function _renderLadderArbCard(bot, botId, container, gameScores, gameKey) {
    ...
    if (bot._obi != null) {
        ...
        else if (absObi < 0.3) obiLabel = `<span style="color:${obi > 0 ? '#00d4ff' : '#ff7043'};">${obi > 0 ? 'YES' : 'NO'} building</span>`;
        else if (absObi < 0.6) obiLabel = `<span style="color:${obi > 0 ? '#00d4ff' : '#ff7043'};">${obi > 0 ? 'YES' : 'NO'} pressure</span>`;
        else obiLabel = `<span style="color:${obi > 0 ? '#00d4ff' : '#ff7043'};">${obi > 0 ? 'YES' : 'NO'} sweep!</span>`;
    }
    ...
    if (hasInv) {
        ...
        if (status === 'mm_exiting' && _activeSell) {
            ...
            // Without this badge the card silently shows "EXIT --" with no signal.
            walkBadge = `<span style="background:#ffaa0022;color:#ffaa00;padding:1px 6px;border-radius:3px;font-size:8px;font-weight:700;animation:pulse 1.5s infinite;">POSTING EXIT</span>`;
        }
        ...
        const exitBox = _sideBox('EXIT', exitBoxSide, effExitPrice > 0 ? `${effExitPrice}c` : 'posting…', effExitPrice > 0 ? `${_fmtN(exitFillQty)}/${_fmtN(exitTotalQty)} filled` : `awaiting exit post`, hedgeFp, `${_fmtN(exitFillQty)}/${_fmtN(exitTotalQty)}`, exitCol, exitBidAsk);
    ...
    item.innerHTML = `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <div style="background:#060a14;border:1px solid #00ff8833;border-radius:8px;padding:10px;">
                <div style="color:#00ff88;font-size:9px;font-weight:800;text-transform:uppercase;margin-bottom:6px;">◎ POSITION · ${side.toUpperCase()}</div>
                ...
                <div style="color:#555;font-size:10px;">cost <strong style="color:#00ff88;">$${costDollars}</strong>${orderFilled && curBidNum > 0 ? ` · value <strong style="color:${curBidNum >= entry ? '#00ff88' : '#ff4444'};">$${valueDollars}</strong>` : ''}</div>
            </div>
            ${isSLSelling ? `
            <div style="background:#060a14;border:1px solid #ff444444;border-radius:8px;padding:10px;">
                <div style="color:#ff4444;font-size:9px;font-weight:800;text-transform:uppercase;margin-bottom:6px;">🛑 SL SELLING · @ ${sellPrice || liveAsk || '?'}¢</div>
                <div style="color:#555;font-size:10px;margin-bottom:4px;">ask <strong style="color:#ff4444;">${liveAsk || '?'}¢</strong> · bid <strong style="color:#ff666688;">${liveBid || '?'}¢</strong></div>
                <div style="color:#ff8800;font-size:10px;font-weight:700;">following ask${sellAge ? ` · ${sellAge}` : ''}</div>
            </div>
            ` : isStopped && exitPrice ? `
            <div style="background:#060a14;border:1px solid ${accentCol}33;border-radius:8px;padding:10px;">
                <div style="color:${accentCol};font-size:9px;font-weight:800;text-transform:uppercase;margin-bottom:6px;">${exitReason === 'stop_loss' ? '🛑 STOP-LOSS' : exitReason === 'take_profit' ? '✅ TAKE-PROFIT' : _scoutSettled ? '🏁 SETTLED' : '⏹ MANUAL'}</div>
                <div style="color:#fff;font-weight:700;font-size:14px;margin-bottom:4px;">${entry}¢ → ${exitPrice}¢</div>
                <div style="color:${exitPnl >= 0 ? '#00ff88' : '#ff4444'};font-size:10px;font-weight:700;">${exitPnl >= 0 ? '+' : ''}${exitPnl}¢ ($${(Math.abs(exitPnl) / 100).toFixed(2)})</div>
            </div>
            ` : `
            <div style="background:#060a14;border:1px solid #ffd74033;border-radius:8px;padding:10px;${!orderFilled ? 'opacity:0.6;' : ''}">
                <div style="color:#ffd740;font-size:9px;font-weight:800;text-transform:uppercase;margin-bottom:6px;">📡 MARKET${hasTPOrder ? ' · TP @ ' + tpPrice + '¢ 📤' : ''}</div>
                <div style="color:#555;font-size:10px;margin-bottom:4px;">bid <strong style="color:#ffd740;">${liveBid || '?'}¢</strong> · ask <strong style="color:#ffd740;">${liveAsk || '?'}¢</strong></div>
                ${sl > 0 || tp > 0 ? `<div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        ${(() => {
            // ONE fill bar — fills on entry, drains on exit
            let barPct, barCol, barLabel;
            if (isStopped) {
                barPct = 0; barCol = accentCol; barLabel = 'SOLD';
            } else if (isSLSelling) {
                barPct = 100; barCol = '#ff4444'; barLabel = watchQty + '× selling';
            } else if (orderFilled) {
                barPct = 100; barCol = '#00ff88'; barLabel = watchQty + '/' + watchQty + ' held';
            } else {
                barPct = fillPct; barCol = fillQty > 0 ? '#ffaa00' : '#333'; barLabel = fillQty + '/' + watchQty;
            }
            return '<div style="display:flex;align-items:center;gap:6px;margin-top:6px;"><div style="flex:1;height:6px;background:#1a2540;border-radius:3px;overflow:hidden;"><div style="width:' + barPct + '%;height:100%;background:' + barCol + ';border-radius:3px;transition:width 0.3s;"></div></div><span style="color:' + barCol + ';font-size:10px;font-weight:700;">' + barLabel + '</span></div>';
        })()}
...
async function phantomModify(botId, restartMode = false) {
    ...
    const _depthValues = [4, 5, 6, 7, 8, 9];
...
function renderApexMMStats(trades, pnl) {
    ...
    statsPanel.innerHTML = lifetimeWins + lifetimeLosses === 0 && !hasAnyFilter
        ...
        : `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            ...
            ${hasAnyFilter
                ? _bub('Win Rate', `${winRate}%`, filterLabel, '#00d4ff')
                : _bub('Win Rate', `${(lifetimeWins + lifetimeLosses) > 0 ? Math.round(lifetimeWins / (lifetimeWins + lifetimeLosses) * 100) : 0}%`, 'lifetime', '#00d4ff')}
            ${hasAnyFilter
                ? _bub('Avg Profit', avgProfit === '—' ? '—' : `${parseFloat(avgProfit) >= 0 ? '+' : ''}${avgProfit}¢`, `${totalTrades} trades`, avgCol)
                : _bub('Avg Profit', (lifetimeWins + lifetimeLosses) > 0 ? `${lifetimePnl >= 0 ? '+' : ''}${(lifetimePnl / (lifetimeWins + lifetimeLosses)).toFixed(1)}¢` : '—', `${lifetimeWins + lifetimeLosses} trades`, lifetimePnlCol)}
            ${hasAnyFilter
    ...
    if (statsPanel) {
        ...
        statsPanel.innerHTML = lifetimeWins + lifetimeLosses === 0 && !hasAnyFilter
            ...
            : `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                ...
                ${hasAnyFilter
                    ? _bub('Win Rate', `${winRate}%`, filterLabel, '#ffaa00')
                    : _bub('Win Rate', `${(lifetimeWins + lifetimeLosses) > 0 ? Math.round(lifetimeWins / (lifetimeWins + lifetimeLosses) * 100) : 0}%`, 'lifetime', '#ffaa00')}
                ${hasAnyFilter
                    ? _bub('Avg Profit', avgProfit === '—' ? '—' : `${parseFloat(avgProfit)>=0?'+':''}${avgProfit}¢`, `${totalTrades} trades`, avgCol)
                    : _bub('Avg Profit', (lifetimeWins + lifetimeLosses) > 0 ? `${lifetimePnl >= 0 ? '+' : ''}${(lifetimePnl / (lifetimeWins + lifetimeLosses)).toFixed(1)}¢` : '—', `${lifetimeWins + lifetimeLosses} trades`, lifetimePnlCol)}
                ${hasAnyFilter
        ...
        depthPanel.innerHTML = depths.length === 0 ? '' : `
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:8px;">
                ${depths.map(d => {
                    ...
                    // v12 ladder (2026-04-28): WALL/PRIME=5c, TRAP=6c, DEEP=7c, FLOOR=8c. 4c and 9c retired.
                    const _tierLabel = d.depth <= 4 ? 'RETIRED' : d.depth === 5 ? 'WALL/PRIME' : d.depth === 6 ? 'TRAP' : d.depth === 7 ? 'DEEP' : d.depth === 8 ? 'FLOOR' : d.depth >= 9 ? 'RETIRED' : '';
                    const _tierCol = d.depth <= 4 ? '#5a6484' : d.depth === 5 ? '#00ccff' : d.depth === 6 ? '#ff8800' : d.depth === 7 ? '#ff6600' : d.depth === 8 ? '#ff4400' : d.depth >= 9 ? '#5a6484' : '#555';
